...
class Scaffold_Container
{
    ...
    $cacheDir = empty($this->options['cache_path']) ? $this->system.'/cache/' : $this->options['cache_path'];
    mkdir($cacheDir);
    if(!is_dir($cacheDir))
      throw new InvalidArgumentException ('Cache dir cannot be created: ' . $cacheDir);

		return $this->_cache = new Scaffold_Cache_File($cacheDir, $this->options['max_age']);
	}
}